...
TEST_CASE("Raycast3D hits a collider without Rigidbody3D") {
    Scene scene;
    Entity sphere = scene.CreateEntity("StaticColliderOnly");
    sphere.GetComponent<TransformComponent>().Translation = { 0.0f, 0.0f, -100.0f };
    sphere.AddComponent<SphereCollider3DComponent>().Radius = 20.0f;
    ...
    CHECK(hit);
    CHECK_SOFT(hit.HitEntity == sphere, "Collider3D alone is registered as static raycast geometry");
}
...
TEST_CASE("Raycast2D hits a collider without Rigidbody2D") {
    Scene scene;
    Entity circle = scene.CreateEntity("StaticColliderOnly");
    circle.GetComponent<TransformComponent>().Translation = { 0.0f, 100.0f, 0.0f };
    circle.AddComponent<CircleCollider2DComponent>().Radius = 20.0f;
    ...
    CHECK(hit);
    CHECK_SOFT(hit.HitEntity == circle, "Collider2D alone is registered as static raycast geometry");
}
...
TEST_CASE("ScreenPointToRay3D fails with no camera and supports Orthographic cameras") {
    Scene scene;
    glm::vec3 origin{}, direction{};
    ...
    CHECK(scene.ScreenPointToRay3D(Screen::Bottom, { BottomScreenWidth * 0.5f, BottomScreenHeight * 0.5f }, origin, direction));
    CHECK_SOFT(glm::distance(origin, glm::vec3(0.0f)) < 0.01f, "center ray starts at the orthographic camera position");
    CHECK_SOFT(glm::distance(direction, glm::vec3(0.0f, 0.0f, -1.0f)) < 0.01f, "orthographic ray is parallel to camera forward");

    glm::vec3 cornerOrigin{}, cornerDirection{};
    CHECK(scene.ScreenPointToRay3D(Screen::Bottom, { 0.0f, 0.0f }, cornerOrigin, cornerDirection));
    CHECK_SOFT(glm::distance(cornerDirection, direction) < 0.01f, "all orthographic screen rays are parallel");
    CHECK_SOFT(glm::distance(cornerOrigin, origin) > 0.01f, "orthographic ray origin changes with screen position");
}
