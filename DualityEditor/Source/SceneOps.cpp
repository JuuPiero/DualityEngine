...
namespace Duality {
    ...
    namespace {
        void ClearRestoredSelection(EditorContext& ctx) {
            // Entity handles are intentionally regenerated by SceneSerializer, so selections
            // from the previous snapshot are always stale after Undo/Redo.
            ctx.Selected = Entity{};
    ...
    bool UndoScene(EditorContext& ctx) {
        if (ctx.IsPlaying || !ctx.History.Undo(ctx.SceneRef))
            return false;
        ClearRestoredSelection(ctx);
        ctx.SceneDirty = !ctx.History.IsAtSavedState();
        return true;
    }

    bool RedoScene(EditorContext& ctx) {
        if (ctx.IsPlaying || !ctx.History.Redo(ctx.SceneRef))
            return false;
        ClearRestoredSelection(ctx);
        ctx.SceneDirty = !ctx.History.IsAtSavedState();
        return true;
    ...
    void OpenScene(EditorContext& ctx, const std::string& path) {
        ...
        ctx.History.Reset(ctx.SceneRef);
        ctx.SceneDirty = false;
    }
