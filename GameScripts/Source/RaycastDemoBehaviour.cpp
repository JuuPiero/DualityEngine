...
void RaycastDemoBehaviour::OnUpdate(float deltaTime) {
    bool down = Duality::ScriptInput::GetPointerDown();
    ...
    // local pixel ranges (see ScriptInput::GetPointerScreen's own comment).
    if (!justPressed || Duality::ScriptInput::GetPointerScreen() != Duality::Screen::Bottom)
        return;

    glm::vec3 origin, direction;
    if (!Duality::ScriptPhysics3D::ScreenPointToRay(Duality::Screen::Bottom, Duality::ScriptInput::GetPointerPosition(), origin, direction)) {
        Duality::ScriptDebug::LogWarn("RaycastDemo: Bottom screen has no Perspective primary camera to raycast from");
        return;
    }

    Duality::RaycastHit3D hit = Duality::ScriptPhysics3D::Raycast(origin, direction, 2000.0f);
    if (hit)
